...
PerlinNoiseShader::PerlinNoiseShader(ID3D11Device* _device, HWND hwnd, int w, int h, int d) : BaseShader(_device, hwnd)
{
	device = _device;
	texWidth = w;
	texHeight = h;
	...
	initShader(L"perlinNoise_cs.cso", NULL);
}
...
void PerlinNoiseShader::setShaderParameters(ID3D11DeviceContext* dc)
{
	// Set UAVs
...
void PerlinNoiseShader::createGPUViews()
{
...
void PerlinNoiseShader::unbind(ID3D11DeviceContext* dc)
{
...
void PerlinNoiseShader::initShader(const wchar_t* cfile, const wchar_t* blank)
{
